package com.schematical.chaoscraft.inventory;
...
/**
 * Created by user1a on 2/22/19.
 */
public class InventoryOrganism implements IInventory {

  /**
   * An array of 36 item stacks indicating the main player inventory (including the visible bar).
   */
  public final NonNullList<ItemStack> mainInventory = NonNullList.<ItemStack>withSize(36,
      ItemStack.EMPTY);
  /**
   * An array of 4 item stacks containing the currently worn armor pieces.
   */
  public final NonNullList<ItemStack> armorInventory = NonNullList.<ItemStack>withSize(4,
      ItemStack.EMPTY);
  public final NonNullList<ItemStack> offHandInventory = NonNullList.<ItemStack>withSize(1,
      ItemStack.EMPTY);
  ...
  public InventoryOrganism(EntityOrganism _entityOrganism) {
    this.allInventories = Arrays.<NonNullList<ItemStack>>asList(this.mainInventory,
        this.armorInventory, this.offHandInventory);
  ...
  private boolean canMergeStacks(ItemStack stack1, ItemStack stack2) {
    return !stack1.isEmpty() && this.stackEqualExact(stack1, stack2) && stack1.isStackable()
        && stack1.getCount() < stack1.getMaxStackSize() && stack1.getCount() < this
  ...
  private boolean stackEqualExact(ItemStack stack1, ItemStack stack2) {
    return stack1.getItem() == stack2.getItem() && (!stack1.getHasSubtypes()
        || stack1.getMetadata() == stack2.getMetadata()) && ItemStack
  ...
  public int getSlotFor(ItemStack stack) {
    for (int i = 0; i < this.mainInventory.size(); ++i) {
      if (!((ItemStack) this.mainInventory.get(i)).isEmpty() && this
          .stackEqualExact(stack, this.mainInventory.get(i))) {
        return i;
      }
    }
  ...
  public int findSlotMatchingUnusedItem(ItemStack p_194014_1_) {
    for (int i = 0; i < this.mainInventory.size(); ++i) {
      ...
      if (!((ItemStack) this.mainInventory.get(i)).isEmpty() && this
          .stackEqualExact(p_194014_1_, this.mainInventory.get(i))
          && !((ItemStack) this.mainInventory.get(i)).isItemDamaged() && !itemstack
          .isItemEnchanted() && !itemstack.hasDisplayName()) {
        return i;
      }
    }
  ...
  public int clearMatchingItems(@Nullable Item itemIn, int metadataIn, int removeCount,
    ...
    for (int j = 0; j < this.getSizeInventory(); ++j) {
      ...
      if (!itemstack.isEmpty() && (itemIn == null || itemstack.getItem() == itemIn) && (
          metadataIn <= -1 || itemstack.getMetadata() == metadataIn) && (itemNBT == null || NBTUtil
          .areNBTEquals(itemNBT, itemstack.getTagCompound(), true))) {
        int k = removeCount <= 0 ? itemstack.getCount()
    ...
    if (!this.itemStack.isEmpty()) {
      ...
      if (itemNBT != null && !NBTUtil
          .areNBTEquals(itemNBT, this.itemStack.getTagCompound(), true)) {
        return i;
      }

      int l = removeCount <= 0 ? this.itemStack.getCount()
  ...
  private int addResource(int p_191973_1_, ItemStack p_191973_2_) {
    ...
    if (itemstack.isEmpty()) {
      itemstack = p_191973_2_
          .copy(); // Forge: Replace Item clone above to preserve item capabilities when picking the item up.
  ...
  /**
   * Decrement the number of animations remaining. Only called on client side. This is used to
   * handle the animation of receiving a block.
   */
  public void decrementAnimations() {
      ...
      for (int i = 0; i < nonnulllist.size(); ++i) {
        if (!((ItemStack) nonnulllist.get(i)).isEmpty()) {
          ((ItemStack) nonnulllist.get(i))
              .updateAnimation(this.entityOrganism.world, this.entityOrganism, i,
                  this.currentItem == i);
        }
      }
    }
    for (ItemStack is : armorInventory) // FORGE: Tick armor on animation ticks
    {
      if (!is.isEmpty()) {
        //TODO: Make this work
        //is.getItem().onArmorTick(entityOrganism.world, entityOrganism, is);
      }
    }
  }
  ...
  public void placeItemBackInInventory(World p_191975_1_, ItemStack p_191975_2_) {
    if (!p_191975_1_.isRemote) {
      while (!p_191975_2_.isEmpty()) {
        ...
        if (i == -1) {
          this.entityOrganism.dropItem(p_191975_2_.getItem(), p_191975_2_.getCount());
          break;
        }
  ...
  public ItemStack decrStackSize(int index, int count) {
    ...
    return list != null && !((ItemStack) list.get(index)).isEmpty() ? ItemStackHelper
        .getAndSplit(list, index, count) : ItemStack.EMPTY;
  }
  ...
  public void damageArmor(float damage) {
    ...
    for (int i = 0; i < this.armorInventory.size(); ++i) {
      ...
      if (itemstack.getItem() instanceof ItemArmor) {
        itemstack.damageItem((int) damage, this.entityOrganism);
      }
    }
  }
  ...
  public void dropAllItems() {
    for (List<ItemStack> list : this.allInventories) {
      for (int i = 0; i < list.size(); ++i) {
        ...
        if (!itemstack.isEmpty()) {
          this.entityOrganism.dropItem(itemstack.getItem(), itemstack.getCount()/*, false*/);
  ...
  public boolean isUsableByPlayer(EntityPlayer player) {
    if (this.entityOrganism.isDead) {
      return false;
    } else {
      return player.getDistanceSq(this.entityOrganism) <= 64.0D;
    }
  }
   ...
   * given slot. For guis use Slot.isItemValid
   */
