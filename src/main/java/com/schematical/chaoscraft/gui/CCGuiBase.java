...
public abstract class CCGuiBase extends GuiScreen {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    fontRenderer.drawString(title,
        this.guiLeft + (this.guiWidth - this.fontRenderer.getStringWidth(title)) / 2,
        this.guiTop + 10, 0x000000);
  ...
  protected void actionPerformed(GuiButton button) throws IOException {
    ...
    switch (ccButton.action) {
      ...
      case VIEW_BIOLOGY_DETAIL_ACTION:
        view = new CCOrgBiologyDetailView(ccButton.entity,
            ((CCOrgBiologyListView.CCBiologyDetailButton) ccButton).biologyBase);
        mc.displayGuiScreen(view);
        return;
