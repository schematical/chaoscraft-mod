        ...
        switch (outputNeuron._outputGroup) {
          ...
          default:
            if (!outputGroupResults.containsKey(outputNeuron._outputGroup)) {
              ...
              OutputGroupResult outputGroupResult = outputGroupResults
                  .get(outputNeuron._outputGroup);
  ...
  public void parseData(JSONObject jsonObject) {
    try {
      ...
      while (iterator.hasNext()) {
        ...
        if (type == null) {
          throw new Error(
              "Invalid Neuron. Missing `$TYPE` - Org.namepace: " + entity.getCCNamespace()
                  + " - Neuron: " + neuronBaseJSON.toJSONString());
        }
        String clsName = type.toString();  // use fully qualified name

        String fullClassName =
            "com.schematical.chaoscraft.ai." + neuronBaseJSON.get("_base_type").toString() + "s."
                + clsName;
        //ChaosCraft.logger.info("Full Class name: " + fullClassName);
        Class cls = Class.forName(fullClassName);
