...
public abstract class NeuronBase extends InnovationBase {
  ...
  public float evaluate() {
    ...
    if (nNet.neuronEvalDepth > 15) {
      throw new ChaosNetException(
          "Max Eval Depth Hit: " + this.nNet.entity.getCCNamespace() + "   " + this.id);
    }
    float totalScore = 0;
    for (NeuronDep neuronDep : dependencies) {
      if (neuronDep.depNeuron == null) {
        ...
        throw new ChaosNetException(
            "Missing `neuronDep.depNeuron` : " + orgNamespace + " " + neuronDep.depNeuronId);
      }
  ...
  public void setDistanceFromIO(int distanceFromOutput, int width, int height, int neuronSize,
      HashMap<NeuronBase, CCOrgNNetView.CCNeuronInformation> neuronInformation) {
    ...
    for (NeuronDep dependency : this.dependencies) {
      dependency.depNeuron
          .setDistanceFromIO(distanceFromOutput + 1, width, height, neuronSize, neuronInformation);
      information.distanceFromInput = Math.max(information.distanceFromInput,
          neuronInformation.get(dependency.depNeuron).distanceFromInput + 1);
    }

    if (information.distanceFromInput == 0 && information.distanceFromOutput == 0) {
      information.x = neuronSize;
    } else {
      information.x = (width - neuronSize * 2) / (information.distanceFromInput
          + information.distanceFromOutput) * information.distanceFromInput + neuronSize;
    }

    System.out.println(this.getClass() + " is " + (!(this instanceof InputNeuron
        || this instanceof OutputNeuron)));

    if (!(this instanceof InputNeuron || this instanceof OutputNeuron)) {
      // this.y = averageY;
      information.y = (int) (Math.random() * height);
      information.layer = information.distanceFromInput / (float) (information.distanceFromInput
          + information.distanceFromOutput);
    }
  }
}
