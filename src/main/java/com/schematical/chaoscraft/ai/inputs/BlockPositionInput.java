...
public class BlockPositionInput extends InputNeuron {
  ...
  public float evaluate() {
    ...
    PositionRange adustedPositionRange = this.blockPositionSensor.positionRange
        .orientToEntity(this.nNet.entity);

    for (float x = adustedPositionRange.minX; x < adustedPositionRange.maxX && _lastValue != 1;
        x++) {
      for (float y = adustedPositionRange.minY; y < adustedPositionRange.maxY && _lastValue != 1;
          y++) {
        for (float z = adustedPositionRange.minZ; z < adustedPositionRange.maxZ && _lastValue != 1;
            z++) {
