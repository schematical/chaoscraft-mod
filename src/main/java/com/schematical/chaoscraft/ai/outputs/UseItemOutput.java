...
public class UseItemOutput extends OutputNeuron {
  ...
  public void execute() {
    ...
    if (rcResult.getType().equals(EnumActionResult.SUCCESS)) {
      ChaosCraft.chat(
          nNet.entity.getCCNamespace() + " successfully rightClicked " + heldItem.getRegistryName()
              .toString());
    } else if (rcResult.getType().equals(EnumActionResult.SUCCESS)) {
      ChaosCraft.chat(
          nNet.entity.getCCNamespace() + " failed to rightClick " + heldItem.getRegistryName()
              .toString());
    }
    ...
    if (result.equals(EnumActionResult.SUCCESS)) {
      ChaosCraft.chat(
          nNet.entity.getCCNamespace() + " successfully used " + heldItem.getRegistryName()
              .toString());
    } else if (result.equals(EnumActionResult.FAIL)) {
      ChaosCraft.chat(
          nNet.entity.getCCNamespace() + " failed to use " + heldItem.getRegistryName().toString());
    }
  }
    /*@Override
