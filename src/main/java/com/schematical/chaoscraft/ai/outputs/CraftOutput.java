...
public class CraftOutput extends OutputNeuron {
  ...
  public float evaluate() {
    ...
    if (nNet.entity.getDebug()) {
      ChaosCraft.logger.info(
          nNet.entity.getCCNamespace() + " Checking to see if they can Craft: " + recipe
              .getRegistryName() + " - " + recipe.getRecipeOutput().getDisplayName());
    }
  ...
  public void execute() {
    ...
    if (nNet.entity.getDebug()) {
      ChaosCraft.logger.info(
          nNet.entity.getCCNamespace() + " Attempting to Craft: " + recipe.getRegistryName() + " - "
    ...
    if (outputStack == null) {
      throw new ChaosNetException(
          "Something went wrong crafting: " + recipe.getRegistryName().toString()
              + " this should not be possible with the `evaluate` check above");
    }
    ...
    String message =
        nNet.entity.getCCNamespace() + " Crafted Recipe: " + recipe.getRegistryName().toString()
            + " - Item: " + worldEvent.item.getRegistryName();
