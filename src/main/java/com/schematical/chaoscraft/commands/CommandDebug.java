...
public class CommandDebug extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        sender.sendMessage(new TextComponentString(chunks.keys().elementSet().size() + " chunks loaded " + chunkToAmount.size()));

        for(ChunkPos pos : chunkToAmount.keySet()) {
            sender.sendMessage(new TextComponentString("     " + pos.toString() + " has been loaded " + chunkToAmount.get(pos) + " times"));
        }
        ...
        for(Entity e : sender.getEntityWorld().loadedEntityList) {
            if(e instanceof EntityOrganism) {
                ...
                if(((EntityOrganism)e).chunkTicket == null) {
                    sender.sendMessage(new TextComponentString("ERROR ORGANISM IS MiSSING TICKET " + e.getName()));
                }
