...
public class CAIMessageHandler implements IMessageHandler<CCIMessage, IMessage> {
  ...
  @Override
  public IMessage onMessage(CCIMessage message, MessageContext ctx) {
    ...
    ChaosCraft.topLeftMessage +=
        "Score: " + payload.get("score") + " - Age: " + payload.get("age") + " / " + payload
            .get("maxAge") + "\n";
    while (jsonObjectIterable.hasNext()) {
      JSONObject ouputValue = (JSONObject) jsonObjectIterable.next();
      ChaosCraft.topLeftMessage +=
          ouputValue.get("summary") + " = " + ouputValue.get("_lastValue") + "\n";
    }
